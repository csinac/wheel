...
namespace RectangleTrainer.WheelOfPseudoFortune.Renderer
{
    public static class WorldSliceMaker
    {
        ...
        public static GameObject GenerateGameObject(float arc, float radius = 1, float thickness = 0.1f, int arcResolution = 8)
        {
            return CreateGameObject(Generate(arc, radius, thickness, arcResolution));
        }

        private static GameObject CreateGameObject(Mesh mesh)
        {
